...
| Option chosen, write ADR | Mark `design` step as `[x]`, then write an ADR using `.opencode/templates/spec/adr-format.md` to the story's `decisions/` subfolder. Pre-populate the ADR's "Rejected" table with the un-chosen options from step 3. Commit includes the ADR file. |
