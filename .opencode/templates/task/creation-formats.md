...
Bug tasks do NOT pre-plan TDD steps at creation time. The progress file starts with a discovery sequence; concrete TDD steps are inserted by `/continue` once `steps-discovery` resolves (see Task Workflow in `.opencode/rules/workflow.md`). This applies to every bug regardless of affected layer -- backend, frontend, or both.
